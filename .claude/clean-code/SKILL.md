...
function update<T extends Record<string, unknown>>(base: T, patch: DeepPartial<T>): T {
  /* ... */
}
...
function updateSettings(current: UserSettings, changes: Partial<UserSettings>): UserSettings {
  /* ... */
}
```
...
function paidRevenue(orders) {
  return orders.filter(o => o.status === 'paid').reduce((sum, o) => sum + o.total, 0)
}
```
